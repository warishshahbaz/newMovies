...
function App() {
  ...
  const fetchData = (searchStr = 'iron') => {
    ...
    let url = ` https://www.omdbapi.com/?s=${searchStr == '' ? 'iron' : searchStr}&apikey=${Apikey}`
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
        console.log(data)
  ...
  return (
    <>
      ...
      <Routes>
        <Route exact path='./home' index element={<Header inputData={inputData} />} />
     ...
     { NewreviewCard && <Reviewscard selected={selected} /> }
      {
        items.map((ele, index) => {
          ...
          return newDiv.innerHTML = <Main size={ele} key={index} reviewCard={ReviewCard} setOnSelected={setOnSelected} /> 
        })
      }
